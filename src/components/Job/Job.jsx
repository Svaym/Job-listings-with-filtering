...
export default function Job() {
  ...
  //  Фильтрация вакансий
  const filteredData = selectedDirection ? data.filter((elem) => elem.languages.includes(selectedDirection)) : data
  return (
    <>
      <section className="">
            ...
            <div className="w-full bg-white shadow-xl rounded-md h-20 relative -top-8 flex items-center justify-between p-4 animate-in slide-in-from-left duration-500">
              ...
              {filters.map((elem, index) => (
                <>
                  ...
                  {elem.title.el}
                    <span className="rounded-l-md p-2 text-darkCyan bg-lightCyan">{elem.title}</span>
                    <button onClick={() => removeFilters(elem.title)} className="p-[13px] bg-darkCyan rounded-r-md transition-colors duration-300 ease-linear hover:bg-veryDarkCyan">
              ...
              <>
                <div className={id===0 || id===1 ? "flex items-center justify-between rounded-md px-7 py-5 bg-white relative after:block after:absolute after:w-2 after:h-full after:left-0 after:bg-darkCyan shadow-xl" : "flex items-center justify-between rounded-md px-7 py-5 bg-white shadow-xl"}>
                  <div className="flex items-center gap-x-5">
                    ...
                    <div className="flex gap-x-3">{item.languages.map((el, i) => (
                      <>
                        <Button key={i} onClick={() => addFilters(el)} cursorPointer='cursor-pointer'>{el}</Button>
                      </>
                      ))}
                    </div>
                </div>
